import { Habitacion } from './habitacion';
import { TipoHabitacion } from './tipoHabitacion';
import { Extras } from './extras';
import { Complemento } from './complemento';
import { Camas } from './camas';
import { Capacidad } from './capacidad';

import { Categoria } from './categoria';
import { Hotel } from './hotel';
export class GeneradorHoteles {
    private _arrayNombreHoteles = [];

    private _habitacion1 = [new Habitacion(new TipoHabitacion((this.obtenerCapacidad(this.getRandom(0, 2))), (this.obtenerCama(this.getRandom(0, 2))), new Complemento(this.obtenerExtras(this.getRandom(0, 4)))), this.getRandom(0, 1000), "data:image/jpeg;base64,")];
    private _habitacion2 = [new Habitacion(new TipoHabitacion((this.obtenerCapacidad(this.getRandom(0, 2))), (this.obtenerCama(this.getRandom(0, 2))), new Complemento(this.obtenerExtras(this.getRandom(0, 4)))), this.getRandom(0, 1000), "")];
    private _habitacion3 = [new Habitacion(new TipoHabitacion((this.obtenerCapacidad(this.getRandom(0, 2))), (this.obtenerCama(this.getRandom(0, 2))), new Complemento(this.obtenerExtras(this.getRandom(0, 4)))), this.getRandom(0, 1000), "")];
    private _habitacion4 = [new Habitacion(new TipoHabitacion((this.obtenerCapacidad(this.getRandom(0, 2))), (this.obtenerCama(this.getRandom(0, 2))), new Complemento(this.obtenerExtras(this.getRandom(0, 4)))), this.getRandom(0, 1000), "")];




    public getHoteles(): Hotel[] {

        this._arrayNombreHoteles.push(new Hotel("Hotel Parissi", (this.obtenerCategoria(this.getRandom(0, 4))), this._habitacion1));
        this._arrayNombreHoteles.push(new Hotel("Hotel Realeza", (this.obtenerCategoria(this.getRandom(0, 4))), this._habitacion1));
        this._arrayNombreHoteles.push(new Hotel("Hotel Mulan", (this.obtenerCategoria(this.getRandom(0, 4))), this._habitacion2));
        this._arrayNombreHoteles.push(new Hotel("Hotel Platinum", (this.obtenerCategoria(this.getRandom(0, 4))), this._habitacion2));
        this._arrayNombreHoteles.push(new Hotel("Hotel Bjusjine", (this.obtenerCategoria(this.getRandom(0, 4))), this._habitacion3));
        this._arrayNombreHoteles.push(new Hotel("Hotel Wine", (this.obtenerCategoria(this.getRandom(0, 4))), this._habitacion3));
        this._arrayNombreHoteles.push(new Hotel("Hotel Caravela", (this.obtenerCategoria(this.getRandom(0, 4))), this._habitacion4));
        this._arrayNombreHoteles.push(new Hotel("Hotel Sergio", (this.obtenerCategoria(this.getRandom(0, 4))), this._habitacion4));
        this._arrayNombreHoteles.push(new Hotel("Hotel Ashera", (this.obtenerCategoria(this.getRandom(0, 4))), this._habitacion3));
        this._arrayNombreHoteles.push(new Hotel("Hotel Dominos", (this.obtenerCategoria(this.getRandom(0, 4))), this._habitacion4));

        return this._arrayNombreHoteles;
    }
    public getRandom(min: number, max: number): number {
        return Math.round(Math.random() * (max - min) + min);
    }

    public obtenerCategoria(numeroEstrellas: number): Categoria {
        let categoria;
        categoria = Categoria[numeroEstrellas];
        return categoria;

    }

    public obtenerCama(numeroCama: number): Camas {
        let cama;
        cama = Camas[numeroCama];
        console.log(cama);
        return cama;

    }
    public obtenerCapacidad(numeroCapacidad: number): Capacidad {
        let capacidad;
        capacidad = Camas[numeroCapacidad];
        return capacidad;

    }
    public obtenerExtras(numeroCapacidad: number): Extras[] {

        let generarExtras;
        let bandera = false;
        let arrayExtras = [];

        if (numeroCapacidad != 0) {
            for (let i = 0; i < numeroCapacidad; i++) {
                if (bandera === false) {
                    generarExtras = Extras[this.getRandom(0, 3)];
                    arrayExtras.push(generarExtras);
                    bandera = true;
                } else {

                    for (let index = 0; index < arrayExtras.length; index++) {
                        generarExtras = Extras[this.getRandom(0, 3)];
                        if (generarExtras != arrayExtras[index]) {
                            arrayExtras.push(generarExtras);
                        }
                    }
                }
            }
        }
        return arrayExtras;

    }


}

